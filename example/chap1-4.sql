...
-- 자바식 : comment_text.substring(0,10) 0번 인덱스부터 10번 인덱스까지
SELECT
    comment_text AS "원본 댓글",
    SUBSTR(comment_text, 1, 10) || '...' AS "댓글 미리보기", -- 1번 index부터 10글자
    CONCAT(user_id, '번 사용자의 댓글') AS "작성자 정보"
-- user_id || '번 사용자의 댓글' AS "작성자 정보" (||은 오라클 전용)
FROM
    COMMENTS
WHERE
...
SELECT
    ...
    REPLACE(p.content, '#일상', '#데일리') AS "해시태그 교체" -- 파마리터를 2개 넣으면 앞에 것을 다 지워버림
    , p.CONTENT AS "원본 게시글"
FROM
    USERS u, POSTS p
WHERE
...
SELECT
    ...
    FLOOR(3.14) AS "인기점수 (내림)", -- FLOOR는 자릿수 지정 불가
...
SELECT
    username,
    registration_date,
    TO_CHAR(registration_date, 'YYYY"년" MM"월" DD"일" MI"분"') AS "가입일 (상세)", -- SQL에서 분 MI
